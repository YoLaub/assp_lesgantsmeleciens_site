import crypto from 'crypto';
import { prisma } from './prisma';
import type { Categorie } from '@/features/adhesion/domain/models/inscription.model';

const CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';

export async function genererNumeroMembreUnique(): Promise<string> {
  let numero: string;
  let exists: boolean;
  do {
    const bytes = crypto.randomBytes(5);
    const suffix = Array.from(bytes).map((b) => CHARS[b % CHARS.length]).join('');
    numero = `ADH-${suffix}`;
    const found = await prisma.membre.findUnique({ where: { numeroAdherent: numero }, select: { id: true } });
    exists = found !== null;
  } while (exists);
  return numero;
}

export function calculerCategorie(dateDeNaissance: Date): Categorie {
  const today = new Date();
  let age = today.getFullYear() - dateDeNaissance.getFullYear();
  const moisDiff = today.getMonth() - dateDeNaissance.getMonth();
  if (moisDiff < 0 || (moisDiff === 0 && today.getDate() < dateDeNaissance.getDate())) age--;
  if (age < 12) return 'enfant';
  if (age < 18) return 'ados';
  return 'adulte';
}
